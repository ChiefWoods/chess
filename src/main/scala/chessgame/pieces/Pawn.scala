...
case class Pawn(private val team: Team.Team, private val piecePosition: Int, private val isFirstMove: Boolean = true) extends Piece(team, PAWN, piecePosition, isFirstMove) {
			...
			if (Board.isValidTileCoordinate(candidateDestinationCoordinate)) {
				if (currentCandidateOffset == 8 && !board.getTile(candidateDestinationCoordinate).isTileOccupied) {
					if (team.isPawnPromotionTile(candidateDestinationCoordinate)) {
						...
						legalMoves += PawnMove(board, candidateDestinationCoordinate, this)
					}
					...
					if (board.getTile(candidateDestinationCoordinate).isTileOccupied) {
						...
						if (team != pieceAtDestination.getPieceTeam) {
							if (team.isPawnPromotionTile(candidateDestinationCoordinate)) {
								legalMoves += PawnPromotion(PawnMove(board, candidateDestinationCoordinate, this), board, candidateDestinationCoordinate, this, pieceAtDestination)
							} else {
	...
	override def movePiece(move: Move): Pawn = {
		Pawn(move.getMovedPiece.getPieceTeam, move.getDestinationCoordinate)
	}
