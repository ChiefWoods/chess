...
import chessgame.utils.Team.{BLACK, Team}
		...
		if (getPlayerKing.getIsFirstMove && !isInCheck) {
			if (!board.getTile(5).isTileOccupied && !board.getTile(6).isTileOccupied) {
				...
				if (rookTile.isTileOccupied &&
					...
					kingCastles += KingSideCastle(board, 6, getPlayerKing, rookTile.getPiece.asInstanceOf[Rook], rookTile.getTileCoordinate, 5)
				}
			}
			...
			if (!board.getTile(1).isTileOccupied && !board.getTile(2).isTileOccupied && !board.getTile(3).isTileOccupied) {
				...
				if (rookTile.isTileOccupied &&
					...
					kingCastles += QueenSideCastle(board, 2, getPlayerKing, rookTile.getPiece.asInstanceOf[Rook], rookTile.getTileCoordinate, 3)
				}
			}
		}
